...
The like operator '~=' is used to indicate either that a wildcard expression will be generated
or that a match_phrase query will be generated. 

##### Wildcard

A wildcard
expression is one where the user is attempting to match against multiple values or does not
know the entire value being search upon. The like operator will automatically choose to 
generate a wildcard query if the parser sees a '*' or '?' character anywhere within the
value given to the expression.
...
A match phrase query is simply a query looking for a sequence of words within a string. A
match_phrase query will be generated by default when no wildcard characters ('*', '?') are 
present in the expression value.
